#!/usr/bin/env node
// Convert 6-digit hex colors in a CSS file to oklch(...) notation.
// Usage: node convert-hex-to-oklch.js path/to/file.css
...
function hexToOklch(hex) {
  ...
  // Format: use percentage for L and decimals for C and h with deg unit
...
if (require.main === module) {
  ...
  if (!file) {
    console.error("Usage: node convert-hex-to-oklch.js path/to/file.css");
    process.exit(1);
  }
  const replaced = convertFile(file);
  console.log("Replaced", replaced.length, "hex colors. Backup at " + file + ".bak");
  // Print a short summary (first 50 replacements)
