...
/**
 * Generate a fluid clamp() function using Utopia's formula
...
function generateFluidValue(minValue, maxValue, name = "") {
  // Utopia formula parameters
  const minViewportRem = CONFIG.minViewportRem;
  const maxViewportRem = CONFIG.maxViewportRem;

  // Calculate slope: (MaxSize - MinSize) / (MaxWidth - MinWidth)
  const slope = (maxValue - minValue) / (maxViewportRem - minViewportRem);

  // Calculate y-intercept: (-1 * MinWidth) * Slope + MinSize
  const yIntersection = -1 * minViewportRem * slope + minValue;

  // Generate clamp with Utopia's formula: clamp(min, yIntersection + slope * 100vw, max)
  const slopeVw = (slope * 100).toFixed(4);
  const cleanSlopeVw = parseFloat(slopeVw);
  const cleanYIntersection = parseFloat(yIntersection.toFixed(4));

  const clampValue = `clamp(${minValue}rem, ${cleanYIntersection}rem + ${cleanSlopeVw}vw, ${maxValue}rem)`;
